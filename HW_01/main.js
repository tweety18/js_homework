...
//3
const getData = prompt('Введіть кількість рядків більше або дорівнює 2:');
const getNumber = +getData;
const isMinimalRows = getNumber >= 2; //for minimal piramide
let pyramidString = '';

if (isNaN(getNumber) || !isMinimalRows) {
    console.log('Pyramid can not be created');
} else {

    for (let j = 1; j <= getNumber; j++) {
        let spaces = ' '.repeat(getNumber - j);
